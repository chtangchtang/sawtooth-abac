...
do
    ...
    do
        ...
        influx_inspect export -datadir '/mnt/influxdb/data' -waldir '/mnt/influxdb/wal' -database metrics -out "/mnt/influxdb/output/pbft/5node/add+delete_${rate}rate_${times}"
        # Analyse results
        python3 /root/sawtooth-abac/analysis/calculate_time_add+delete.py /mnt/influxdb/output/pbft/5node/add+delete_${rate}rate_${times} /root/pbft_5node_result.csv
        # Down network
        docker-compose -f /root/sawtooth-abac/test/pbft/5nodes.yaml down -v
    done
done
# Compress result files and remove them
tar -czvf /mnt/influxdb/output/pbft_5node_add+delete.tar.gz /mnt/influxdb/output/pbft/5node/
rm /mnt/influxdb/output/pbft/5node/*
