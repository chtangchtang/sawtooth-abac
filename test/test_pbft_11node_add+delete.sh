...
do
    ...
    do
        ...
        influx_inspect export -datadir '/mnt/influxdb/data' -waldir '/mnt/influxdb/wal' -database metrics -out "/mnt/influxdb/output/pbft/11node/add+delete_${rate}rate_${times}"
        # Analyse results
        python3 /root/sawtooth-abac/analysis/calculate_time_add+delete.py /mnt/influxdb/output/pbft/11node/add+delete_${rate}rate_${times} /root/pbft_11node_result.csv
        # Down network
        docker-compose -f /root/sawtooth-abac/test/pbft/11nodes.yaml down -v
    done
done
# Compress result files and remove them
tar -czvf /mnt/influxdb/output/pbft_11node_add+delete.tar.gz /mnt/influxdb/output/pbft/11node/
